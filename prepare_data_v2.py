"""
Fast data prep v2: wget + batched tokenization + text truncation.
...
MAX_SEQ_LEN    = 8192
MAX_TEXT_CHARS = 50000                  # truncate very long texts before tokenizing
...
STAGING_DIR    = DATA_DIR / "_staging_v2"
CHECKPOINT     = DATA_DIR / "prepare_checkpoint_v2.json"
...
def main():
    ...
    print(f"🚀 wget + batched tokenization v2 (truncated long texts)")
                    ...
                    if success and out.exists():
                        ...
                        try:
                            ...
                            for b_start in range(0, len(texts), batch_size):
                                ...
                                # Flatten all token arrays efficiently
                                all_ids = [np.array(ids, dtype=np.uint16) for ids in encoded["input_ids"] if ids]
                                del encoded
                                if not all_ids:
                                    continue
                                flat = np.concatenate(all_ids)
                                del all_ids
                                file_tokens += len(flat)
                                
                                n = len(flat)
                                start = 0
                                while start < n:
                                    remaining = SHARD_SIZE - buf_pos
                                    chunk = flat[start:start+remaining]
                            ...
                            print(f"   ✅ {len(texts):,} rows → {file_tokens:,} tokens in {elapsed:.1f}s ({file_tokens/elapsed:,.0f} tok/s)")
                            del texts
                            gc.collect()
