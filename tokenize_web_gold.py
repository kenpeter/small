#!/usr/bin/env python3
"""Tokenize web_gold reformatted docs (textbook.jsonl + qa.jsonl) into pretraining .bin shards.

Writes uint16 token arrays (no header) into _shards_web_gold/, the same format the
stratified loader reads (np.fromfile(dtype=np.uint16), seq chunks of 2048).
50M tokens per shard.
"""
...
INPUT_FILES = [
    Path("/home/kenpeter/work/data/_web_gold/reformatted/textbook.jsonl"),
    Path("/home/kenpeter/work/data/_web_gold/reformatted/qa.jsonl"),
]
OUT_DIR = Path("/home/kenpeter/work/data/_shards_web_gold")
...
def main():
    ...
    for INPUT_FILE in INPUT_FILES:
        if not INPUT_FILE.exists():
            print(f"  ERROR: missing input {INPUT_FILE}", flush=True)
            sys.exit(1)
        ...
        with open(INPUT_FILE) as f:
            for line in f:
                ...
                while shard_buf and sum(len(a) for a in shard_buf) >= TOKENS_PER_SHARD:
                    ...
                    shard_buf = []
                    out_path = OUT_DIR / f"web_gold-{shard_idx:05d}.bin"
