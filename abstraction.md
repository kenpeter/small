...
| Power limit | **100W — ACTIVE (user-tuned Aug 13; journey 200W→120W→200W→150W→200W→100W→150W→100W — floor of valid range)**, auto-applied at launch by `run_pretrain_gpu_60k.sh` (non-fatal if sudo unavailable). Verified numbers on this card: 285W = ZERO gain (thermal throttle wall, 87–88°C); **200W = speed ceiling** (9.12 s/step / 7,183 tok/s, ~80°C); 150W = ~10.3 s/step, ~73°C; 120W = 11.49 s/step / 5,701 tok/s, 69°C; **100W floor = 14.15 s/step / 4,632 tok/s, ~64°C**. Script-applied → survives reboot/GPU-idle |
