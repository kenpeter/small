...
| `_shards_gold/` | 17.9M tok (35.8 MB) | ~17.9M | **Gold set (Qwen3-8B): 5,390 Qs × 3 methods + 5 variants** | ✅ Active (×5 boost) |

**Notes:**
- Shard format: `.bin` uint16 arrays, SmolLM2-135M vocab (49152)
- **Gold set (2026-08-16):** 400 pilot + 4,990 v2 questions = **5,390 docs → 8,742 seqs** — LeetCode problems (240+3,000 code, with `high_quality_cot`) + finemath score≥2 (160+2,000 math). Each doc = original question + 3 solution methods + 5 similar variants (+ full solution for math), generated by Qwen3-8B-AWQ via vLLM (`gold_gen.py`, resume-able, dedup by question fingerprint). Raw JSONL kept: `data/_gold/gold_set.jsonl` + `gold_set_v2.jsonl` (never delete — regenerable but expensive, ~14h GPU). Tokenized via `tokenize_gold.py` → `_shards_gold/gold-00000.bin`.
...
- **Gold set → LIVE in training (the "repeat 5×" experiment, Phi-1-style):** `gold_hard` domain added (`pretrain_megatrain.py` — SHARD_DIRS + HARD_SPLIT 0.05), **×5 boost** via `curriculum_boost.json` hot-reload (`"gold_hard": 5.0` → ~25% of hard tier ≈ 8-10% of all tokens), DoReMi ref added (`refs.json` `gold_hard: 1.5` — DoReMi upweights if stuck >1.5, tapers to 0.25× once mastered: heavy drill early, auto-governed repetition). SDLC 73/73 before relaunch.
- **Generation run:** 400 pilot (`gold_set.jsonl`) + 4,990 v2 (`gold_set_v2.jsonl`) = **5,390 docs → 8,742 seqs → 17.9M tokens** (`_shards_gold`). 8 workers, ~10h, dedup'd by question fingerprint, ~0.2% API-error skip tolerance. vLLM Qwen3-8B-AWQ offline mode (`HF_HUB_OFFLINE=1` — HF handshake times out on slow QRP link). `<think>`-block stripping in `gold_gen.py`.
- **Training:** ▶️ RUNNING **115K cycle** — resumed **09:11** from `megatrain_latest.pt` (step 62,688) with gold live. `Domain 'gold_hard': 1 shards, 8,742 seqs` confirmed in log. Compile OOM'd at JIT (every launch, as documented) → eager fallback.
- **Checkpoints backed up:** `backup_20260815/` + `backup_20260816/` (latest step 62,688 + best + refs, 12 GB each).
...
- **code_gold → LIVE & sprinting + now FOLDED-IN from the big drive (the "more greedy questions" ask):**
  - Round 1 (this session, earlier): **134 Qwen3-8B-AWQ generated + exec-verified** solutions (`gen_code_gold_qwen.py`), tokenized + 150× identifier-variant replication → `code_gold` bin 148 MB / 74M tok / 36,150 seqs (241 expanded). Added to `pretrain_megatrain.py` SHARD_DIRS + HARD_SPLIT (0.12) + boost `code_gold` 10.0; SDLC 74/74 (commit `e378f3b`). code_gold loss collapsed 1.63 → 0.83.
  - Round 2 (post-pause, fold-in): swept all LeetCode datasets on `/mnt/file_drive/data/` → **`high_quality_leetcode` (2,638 probs with real `completion` + `test` cases) is the gold source** → exec-verified **513 / 2,638 pass their own test cases** (verify_hq_code.py; 231 Med / 191 Easy / 91 Hard; newfacade adds 1 dup → merged 513 unique). Folded via `fold_hq_code_gold.py` → merged corpus **1,074 unique solutions (833 HQ variants + 241 Qwen)** → rebuilt bin **110 MB / 26,850 raw seqs / 771 unique post-13-gram-dedup** (verify_hq_code.py + fold_hq_code_gold.py + build_code_gold_bin.py). Loader smoke-tested: 26,850 seqs, valid samples. **NOTE: 13-gram dedup (front13+back13) collapses all replication** (old bin 36,150→135 unique; new 26,850→771) — replication is defeated by dedup, so the REAL win is unique content: 135 → 771 distinct verified problems (5.7×). Lean factor 25 rebuild avoided wasting 650MB on deduped-away replicas. mesolitica (1,561 Hard) + DenCT (19K) have NO test cases & mesolitica solutions are mismatched → not exec-verifiable directly, skipped (future: run through Qwen→verify).
- **13-gram dedup defeats replication (design note):** every 13-gram-identical seq (all replicas + padded class-format headers/`</s>` tails) collapses to one → code_gold effective pool = unique docs only. High sample ratio (0.12-0.14 in HARD_SPLIT) is what drills the small pool repeatedly. To grow real drilling content, GROW UNIQUE verified docs (as done), not replication factor.
